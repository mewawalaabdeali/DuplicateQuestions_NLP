...
def test_fetch_token_features(q1, q2):
  ...
  token_features[2] = common_stop_count / (min(len(q1_stops), len(q2_words)) + SAFE_DIV)
  token_features[3] = common_stop_count / (max(len(q1_stops), len(q2_words)) + SAFE_DIV)
  token_features[4] = common_token_count / (min(len(q1_tokens), len(q2_words)) + SAFE_DIV)
  token_features[5] = common_token_count / (max(len(q1_tokens), len(q2_words)) + SAFE_DIV)
...
def preprocess(q):
  ...
  #Replace characters with their string equivalents
  q = q.replace('%', 'percent')
  q = q.replace('$', 'dollar')
  q = q.replace('₹', 'rupee')
  q = q.replace('€', 'euro')
